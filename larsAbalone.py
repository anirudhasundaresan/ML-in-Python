...
# code the three-valued sex attribute (M/F/I)
xCoded = [] # final new xList matrix
for row in xList:
    codedSex = [0.0, 0.0] # as it is for 'I' category.(when abalones are infants, their sex is indeterminate)
    if row[0]=='M': codedSex[0] = 1.0
    if row[0]=='F': codedSex[1] = 1.0
    xCoded.append(list(codedSex)+[float(row[i]) for i in range(1, len(row))]) # take codedsex attribute and add it to the existing row, but without the sex attribute.

# recode the names as well
...
nameList = [namesCoded[nzList[i]] for i in range(len(nzList))] # sex2 and sex1 appear indicating that we have successfully used the encoding scheme to add the attribute 'Sex' to our regressor model.
...
'''
Coding the sex attribute is accomplished before the attribute matrix is normalized. (Normalize with encoded values).
